...
func getdownDUrl(url string) ([]byte, error) {
...
func pastDownJson(data []byte) (map[string]interface{}, error) {
...
func GetDownData(flagStr string) []string {
	var list []string
	...
	data, _ := getdownDUrl(url)
	jsxData, err := pastDownJson(data)
	if err != nil {
		log.Println(err)
	}
	for _, v := range jsxData {
		switch vv := v.(type) {
...
//fmt.Println(GetDData(`max(rate(node_network_receive_bytes_total{job=~"host-node"}[30s])*8)by(instance)`, "host-node"))
