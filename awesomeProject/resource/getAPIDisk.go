...
func GetDiskUrl(url string) ([]byte, error) {
...
func pastDiskJson(data []byte) (map[string]interface{}, error) {
...
func GetDiskData(flagStr string) []string {
	var list []string
	...
	data, _ := GetDiskUrl(url)
	jsdatA, err := pastDiskJson(data)
	if err != nil {
		log.Println(err)
	}
...
//fmt.Println(GetDData(`(node_filesystem_size_bytes{job="host-node",mountpoint="/",fstype=~"ext.?|xfs"}-node_filesystem_free_bytes{job="host-node",mountpoint="/",fstype=~"ext.?|xfs"})/node_filesystem_size_bytes{job="host-node",mountpoint="/",fstype=~"ext.?|xfs"}*100`, "host-node"))
