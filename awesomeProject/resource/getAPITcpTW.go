...
func getDUrltw(url string) ([]byte, error) {
...
func pastTJson(data []byte) (map[string]interface{}, error) {
...
func GetTCPTW(flagStr string) []string {
	var list []string
	...
	data, _ := getDUrltw(url)
	jsTdata, err := pastTJson(data)
	if err != nil {
		log.Println(err)
	}
	for _, v := range jsTdata {
		switch vv := v.(type) {
...
//fmt.Println(GetTCPTW(`node_sockstat_TCP_tw{job=~"$job"}-0`, "host-node"))
