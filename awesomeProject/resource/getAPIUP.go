...
func getapiup(url string) ([]byte, error) {
...
func PastupDJson(data []byte) (map[string]interface{}, error) {
...
func GetupData(flagStr string) []string {
	var list []string
	...
	data, _ := getapiup(url)
	jsGdata, err := PastupDJson(data)
	if err != nil {
		log.Println(err)
	}
	for _, v := range jsGdata {
		switch vv := v.(type) {
...
//fmt.Println(GetDData(`max(rate(node_network_transmit_bytes_total{job=~"host-node"}[30s])*8)by(instance)`, "host-node"))
